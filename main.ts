...
function postProcessSoapXML(xml: string): string {
  ...
  return xml
    ...
    .replace(/<(\/)?tns:/g, '<$1q1:').replace('<session_id xsi:type="xsd:string">', "<session_id>")
}
...
console.log(await client.LoginUserAsync({user_id: process.env.PHAROS_USERNAME, password:  process.env.PHAROS_PASSWORD}, { postProcess: postProcessSoapXML }))
